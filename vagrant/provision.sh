...
# Runs ./vagrant/create-dev-env.sh for the actual setup
#
...
on_create()
{
    ...
    mkdir -p /opt/edx/node_modules /opt/edx/pepperpd/node_modules
    ([[ -f /etc/fstab ]] && grep '/opt/edx/node_modules' /etc/fstab) || {
        echo '/opt/edx/node_modules /opt/edx/pepperpd/node_modules none bind,noauto 0 0' >> /etc/fstab
    ...
    ([[ -f /etc/cron.d/nodemodules ]] && grep '/opt/edx/node_modules' /etc/cron.d/nodemodules) || {
        echo '@reboot root until [ -n "`mount |grep "/opt/edx/pepperpd type"`" ]; do sleep 1; done; mount /opt/edx/node_modules' > /etc/cron.d/nodemodules
    }

    # Force rechecking all prerequisites (could have been fetched outside of the VM)
    rm -rf /opt/edx/pepperpd/.prereqs_cache

    # Permissions
    chown vagrant.vagrant /opt/edx /opt/edx/node_modules /opt/edx/pepperpd/node_modules

    # For convenience with `vagrant ssh`, the `pepperpd` virtualenv is always
    ...
    sudo -u vagrant -i bash -c "cd /opt/edx/pepperpd && PROJECT_HOME=/opt/edx ./vagrant/create-dev-env.sh -yq"
    ...
    ([[ -f ~vagrant/.bash_profile ]] && grep "pepperpd/bin/activate" ~vagrant/.bash_profile) || {
	echo ". /home/vagrant/.virtualenvs/pepperpd/bin/activate" >> ~vagrant/.bash_profile
    }


    # Directory ###################################################################

    grep "cd /opt/edx/pepperpd" ~vagrant/.bash_profile || {
	echo "cd /opt/edx/pepperpd" >> ~vagrant/.bash_profile
    }
