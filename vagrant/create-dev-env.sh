...
if [[ -r $BASE/pepperpd/vagrant/ruby-version ]]; then
  RUBY_VER=`cat $BASE/pepperpd/vagrant/ruby-version`
fi
...
if [[ -d $BASE/pepperpd/vagrant ]]; then
    output "Installing system-level dependencies"
    bash $BASE/pepperpd/vagrant/install-system-req.sh
else
...
bundle install --gemfile $BASE/pepperpd/Gemfile
...
if [[ $systempkgs ]]; then
    mkvirtualenv -q -a "$WORKON_HOME" --system-site-packages pepperpd || {
    ...
    # --no-site-packages
    mkvirtualenv -q -a "$WORKON_HOME" pepperpd || {
      error "mkvirtualenv exited with a non-zero error"
      return 1
    }
fi
...
SITE_PACKAGES="$WORKON_HOME/pepperpd/lib/python2.7/site-packages"
cd "$SITE_PACKAGES"
